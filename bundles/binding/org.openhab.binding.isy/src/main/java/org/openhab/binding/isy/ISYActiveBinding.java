...
public class ISYActiveBinding extends AbstractActiveBinding<ISYBindingProvider>implements ISYModelChangeListener {
    ...
    protected void internalReceiveUpdate(final String itemName, final State newState) {
        ...
        // BindingProviders provide a binding for the given 'itemName'.
        if (this.logger.isDebugEnabled()) {
            this.logger.debug("internalReceiveUpdate({},{}) is called!", itemName, newState);
        }
    }
     ...
     * @param action
     *            Mostly a string with the value, e.g. "0", "100"
     * @param node
                ...
                for (ISYBindingConfig config : configs) {
                    ...
                    switch (config.getType()) {
                        ...
                        case CONTACT:
                            ...
                            break;
                        case THERMOSTAT:
                            state = new DecimalType(new DecimalType((String) action).doubleValue());
                            break;
                        case NUMBER:
                        ...
                        case DIMMER:
                            if ("0".equals(action)) {
                                state = OnOffType.OFF;
                            } else if ("100".equals(action)) {
                                state = OnOffType.ON;
                            } else {
                                BigDecimal dim = new BigDecimal(
                                        Math.round(new DecimalType((String) action).doubleValue()));
            ...
            switch (config.getType()) {
                case THERMOSTAT:
                    switch (config.getControlCommand()) {
                        case CLISPH:
                        case CLISPC:
                            DecimalType value = new DecimalType(type.doubleValue());
                ...
                case DIMMER:
                    switch (config.getControlCommand()) {
                        case ST:
                            DecimalType dim = new DecimalType(Math.round(type.doubleValue()));
                            if (dim.intValue() <= 0) {
                                this.insteonClient.turnDeviceFastOff(node.address);
                            } else if (dim.intValue() >= 100) {
                                this.insteonClient.turnDeviceFastOn(node.address);
                            } else {
